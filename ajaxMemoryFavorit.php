...
if(isset($_POST['memoryId']) && isset($_SESSION['user_id']) && isLogin()){
  ...
  try {
    ...
    if (!$stmt) throw new Exception('お気に入り情報が取得できませんでした');
    $resultCount = $stmt->rowCount();
    debug($resultCount);
    // レコードが１件でもある場合
    if(!empty($resultCount)){
      $sql = 'DELETE FROM memory_like WHERE memory_id = :m_id AND user_id = :u_id';
      $data = array(':u_id' => $_SESSION['user_id'], ':m_id' => $m_id);
      // クエリ実行
      $stmt = queryPost($dbh, $sql, $data);
      ...
      $sql = 'INSERT INTO memory_like (memory_id,create_date,user_id) VALUES(:m_id,:date,:u_id)';
